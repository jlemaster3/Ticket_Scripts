...
output_directory = "C:\\Users\\jlemaster3\\OneDrive - Gainwell Technologies\\Documents\\_Ticket_Repo\\RITM0165194\\SIT_SCHEDULE_SWAP"

output_log_file = "C:\\Users\\jlemaster3\\OneDrive - Gainwell Technologies\\Documents\\_Ticket_Repo\\RITM0165194\\SIT_SCHEDULE_SWAP_log.txt"
...
if __name__ == "__main__":
    ...
    _output_log_holder.append('-'*100)
    _output_log_holder.append(f"File Set - A - 'ALT' in File Name : '{len(_file_list_A)}'")
    _output_log_holder.append(f"Number of Sarch Terms A: [{len(search_terms_A)}]")
    for _term_count, _term in enumerate(search_terms_A):
        _output_log_holder.append(f"  [{_term_count+1}] - {_term}")
    _output_log_holder.append(f"Files Set - A - Number of files containing Search Terms: [{len(fitlered_files_A)}]")
    _output_log_holder.append('-'*100)
    ...
    _output_log_holder.append('- File SET - A')
    _output_log_holder.append('-')
    _output_log_holder.append('-'*100)

    process_files(
        path_list=fitlered_files_A, 
        seach_replace_dictionary=find_replace_strings_A, 
