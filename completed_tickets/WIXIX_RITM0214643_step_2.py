...
Duplcaite all Job Streams and Jobs that are defined on MACH1 to also be defined on MACH3
'''
...
source_directory = "C:\\Users\\jlemaster3\\OneDrive - Gainwell Technologies\\Documents\\_Ticket_Repo\\RITM0214643\\SETE_Copy"
...
output_log_file = "C:\\Users\\jlemaster3\\OneDrive - Gainwell Technologies\\Documents\\_Ticket_Repo\\RITM0214643\\WVXIX_RITM0214643_step2_duplcaite_log.txt"
...
exclude_text = [
    ...
    "SYS{E}D160",
    "SYS{E}D161",
    "SYS{E}STPA AS SYSTSTPA",
    "SYS{E}STRA AS SYSTSTRA",
    "SYS{E}D902",
    "SYS{E}M000",
    "SYS{E}M999",
    "SYS{E}WSPC"
...
def add_Draft_to_String (source_string:str, exclusiveList:list[str]=None):
    _lines = str(source_string).split('\n')
    _js_list:list[int] = []
    _description_lines:list[int] = []
    _has_draft:list[int] = []
    _exclusiveTerms:dict[int, list[str]] = {}
    for _id, _line in enumerate(_lines):
        ...
        if exclusiveList != None and len(exclusiveList) >= 1:
            for _term in exclusiveList:
                if _term.lower() in _line.lower():
                    if _id not in _exclusiveTerms.keys():
                        _exclusiveTerms[_id] = []
                    _exclusiveTerms[_id].append(_term)
            ...
            if (len(_has_draft) >= 1):
                for _draft_id in _has_draft:
                    if _set[0] < _draft_id < _set[1] :
                        already_draft = True
            if (exclusiveList != None and len(_exclusiveTerms.keys()) >= 1):
                _skip = True
                for _exclusive_lineID, _termList in _exclusiveTerms.items():
                    if _set[0] < _exclusive_lineID < _set[1] :
                        _output_log_holder.append(f"    - Enabling 'DRAFT' on Job Steam, contains terms : [{', '.join(_termList)}]")
                        _skip = False
                if _skip == True:
                    continue
    ...
    for counter, filePath in enumerate(path_list):
        ...
        try:    
            ...
            if os.path.isfile(filePath):
                ...
                _output_log_holder.append(f" [{counter+1}] - Created new file : {output_filePath}")
...
if __name__ == "__main__":
    ...
    _output_log_holder.append(f"Number of Search and Replace Terms: [{len(find_replace_strings.keys())}]")
