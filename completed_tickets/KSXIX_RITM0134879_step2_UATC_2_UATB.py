...
output_directory = "C:\\Projects\\scripts\\jil_File_tools\\output_path\\KSXIX_RITM0134879\\UATB"

output_log_file = os.path.join(output_directory, 'action_log.txt')
...
def copy_to_path (source_filePath_list:list[str], source_root_path:str, target_output_path:str) ->str:
    ...
    for _fp_counter,_filePath in enumerate(source_filePath_list):
        try:
            ...
            _log_holder.append(f"[{str(_fp_counter+1).zfill(_file_cntr_str_len)}] - Source file not found: {_filePath}")
...
if __name__ == "__main__":
    ...
    _copy_log = copy_to_path(_missing_from_B,source_dir_A, source_dir_B)
    _differences_log, _diff_counter = check_differences(_file_list_A, _file_list_B, source_dir_A, _fileNames_in_both, source_dir_B)
