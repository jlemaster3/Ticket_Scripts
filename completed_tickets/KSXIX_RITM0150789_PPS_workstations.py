...
output_directory = "C:\\Users\\jlemaster3\\OneDrive - Gainwell Technologies\\Documents\\_Ticket_Repo\\RITM0150789\\Diffrences\\PPS"
...
def process_files (path_list:list, seach_replace_dictionary:dict, source_path:str, output_path:str, overwrite_original:bool=False, keep_original:bool=False, original_to_DRAFT:bool=False):
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    for counter, filePath in enumerate(path_list):
        file_name = os.path.basename(filePath)
        if overwrite_original == False:
            _rel_path = os.path.relpath(os.path.dirname(filePath), source_path)
            _output_path = os.path.join(output_path, _rel_path)
            if not os.path.exists(_output_path):
                os.makedirs(_output_path)
            output_filePath = os.path.join(_output_path, file_name)    
        else:
...
if __name__ == "__main__":
    ...
    process_files(fitlered_files, find_replace_strings, source_directory, output_directory)
